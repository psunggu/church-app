...
(function () {
  ...
  function segHtml() {
    return '<div class="seg" style="max-width:520px"><button class="' + (D.tab === 'week' ? 'on' : '') + '" data-act="setTab" data-t="week">주간 현황</button><button class="' + (D.tab === 'trend' ? 'on' : '') + '" data-act="setTab" data-t="trend">추이</button><button class="' + (D.tab === 'parish' ? 'on' : '') + '" data-act="setTab" data-t="parish">구역 상세</button><button class="' + (D.tab === 'term' ? 'on' : '') + '" data-act="setTab" data-t="term">학기표</button></div>';
  }
  ...
  /* ── 학기표 ───────────────────────────────────────────────
     교역자가 엑셀로 보던 표: 행 = 구역, 열 = 학기 안 수요일(달별 묶음), 값 = 그 주 총점.
     주일 출석이 그 주 수요일 칸에 놓인다(9/13 → 9/9, C41.wedOf). 이전 학기 누계는
     term_totals 의 수동값이 있으면 그것, 없으면 기록 합계. 과 번호는 term_weeks.note. */
  async function loadTerm() {
    D.term = null; D.termLoading = true;
    var t = await sb.from('terms').select('*').order('start_date');
    var terms = t.data || [];
    var cur = terms.filter(function (x) { return x.start_date <= D.week && D.week <= x.end_date; })[0] || terms[terms.length - 1];
    if (!cur) { D.termLoading = false; return; }
    var prev = terms.filter(function (x) { return x.end_date < cur.start_date; }).pop() || null;
    var wk = await sb.from('term_weeks').select('*').eq('term_id', cur.id).order('report_date');
    var from = prev ? prev.start_date : cur.start_date;
    var pw = await sb.from('v_parish_week').select('parish_id,report_date,total_score,entered_cnt').gte('report_date', from).lte('report_date', cur.end_date);
    var ld = await sb.from('members').select('parish_id,name').eq('role', '구역장').neq('state', 'Delete');
    var tt = prev ? await sb.from('term_totals').select('parish_id,total').eq('term_id', prev.id) : { data: [] };
    if (pw.error) C41.toast('학기표 조회 실패: ' + C41.errMsg(pw.error));
    D.term = { cur: cur, prev: prev, grid: C41.termGrid(cur, wk.data || []), pw: pw.data || [], leaders: ld.data || [], totals: tt.data || [] };
    D.termLoading = false;
  }
  function termTabHtml() {
    if (D.termLoading || D.term === undefined) return '<div class="msg">불러오는 중...</div>';
    if (!D.term) return '<div class="msg">학기가 등록되어 있지 않습니다 — 관리 화면 → 설정에서 학기와 모임 날짜를 넣으세요</div>';
    var T = D.term, P = D.parishes, today = D.access.today || C41.kstToday(), cols = [];
    T.grid.forEach(function (m) { m.weeks.forEach(function (w) { cols.push(w); }); });
    var ix = {}; T.pw.forEach(function (x) { ix[x.parish_id + '|' + String(x.report_date).slice(0, 10)] = x; });
    var lead = {}; T.leaders.forEach(function (l) { lead[l.parish_id] = l.name; });
    var tot = {}; T.totals.forEach(function (x) { tot[x.parish_id] = x.total; });
    var h = '<div class="ctrl"><span class="info">' + esc(T.cur.name) + ' · 주일 출석이 그 주 <b>수요일</b> 칸에 놓입니다 (9/13 → 9/9)</span></div>';
    h += '<div class="tw"><table class="rt term"><tr><th rowspan="2" class="nm">구역</th><th rowspan="2">구역장</th>'
       + (T.prev ? '<th rowspan="2" class="cum">' + esc(T.prev.name) + '<br>누계</th>' : '');
    T.grid.forEach(function (m) { h += '<th colspan="' + m.weeks.length + '" class="mo">' + m.month + '월</th>'; });
    h += '<th rowspan="2" class="cum">' + esc(T.cur.name) + '<br>누계</th></tr><tr>';
    cols.forEach(function (w) { h += '<th class="' + (w.off ? 'off' : '') + '">' + w.n + '주(' + parseInt(w.wed.slice(8), 10) + ')<br><small>' + (w.off ? '휴강' : esc(w.note || '')) + '</small></th>'; });
    h += '</tr>';
    var sumCol = {}, sumPrev = 0, sumCur = 0; cols.forEach(function (w) { sumCol[w.wed] = 0; });
    P.forEach(function (p) {
      var prevTotal = null;
      if (T.prev) prevTotal = tot[p.id] != null ? tot[p.id] : T.pw.filter(function (x) { return x.parish_id === p.id && x.report_date >= T.prev.start_date && x.report_date <= T.prev.end_date; }).reduce(function (a, x) { return a + C41.n(x.total_score); }, 0);
      var cur = 0;
      h += '<tr><td class="nm">' + esc(p.id) + '</td><td>' + esc(lead[p.id] || '') + '</td>' + (T.prev ? '<td class="sc cum">' + C41.fmt(prevTotal) + '</td>' : '');
      cols.forEach(function (w) {
        if (w.off) { h += '<td class="off"></td>'; return; }
        var v = ix[p.id + '|' + w.sunday], sc = v ? C41.n(v.total_score) : 0, entered = v && C41.n(v.entered_cnt) > 0;
        cur += sc; sumCol[w.wed] += sc;
        h += '<td class="' + (entered ? 'v' : 'z') + '">' + (w.sunday <= today ? C41.fmt(sc) : '') + '</td>';
      });
      sumPrev += prevTotal || 0; sumCur += cur;
      h += '<td class="sc cum">' + C41.fmt(cur) + '</td></tr>';
    });
    h += '<tr class="tr"><td class="nm">' + P.length + '</td><td></td>' + (T.prev ? '<td class="sc">' + C41.fmt(sumPrev) + '</td>' : '');
    cols.forEach(function (w) { h += '<td>' + (w.off || w.sunday > today ? '' : C41.fmt(sumCol[w.wed])) + '</td>'; });
    h += '<td class="sc">' + C41.fmt(sumCur) + '</td></tr></table></div>';
    h += '<div class="rt-note">과 번호와 휴강은 관리 화면 → 설정 → 모임 날짜에서, 이전 학기 누계는 같은 곳의 <b>학기 누계</b>에서 넣습니다. 비어 있으면 기록 합계가 보입니다.</div>';
    return h;
  }
  ...
  function _render() {
    ...
    if (D.tab === 'week') h += weekTabHtml(); else if (D.tab === 'trend') h += trendTabHtml(); else if (D.tab === 'term') h += termTabHtml(); else h += parishTabHtml();
  ...
  var ACTIONS = {
    ...
    setWeek: function (el) { D.week = el.getAttribute('data-w'); D.detail = null; D.term = undefined; loadWeek().then(function () { if (D.tab === 'parish' && D.selParish) loadDetail().then(render); if (D.tab === 'term') loadTerm().then(render); }); },
    setTab: function (el) { D.tab = el.getAttribute('data-t'); render(); if (D.tab === 'parish' && D.selParish && !D.detail) loadDetail().then(render); if (D.tab === 'term' && D.term === undefined) loadTerm().then(render); },
