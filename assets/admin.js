...
(function () {
  ...
  async function loadSettings() {
    ...
    var tt = await sb.from('term_totals').select('*'); A.termTotals = tt.error ? [] : (tt.data || []);   // sql/019 전이면 빈 목록
  ...
  // 과 번호(예: 15과). 학기표 대시보드가 그 주 수요일 칸 밑에 보여 준다.
  async function saveWeekNote(termId, d) {
    var el = document.getElementById('wn-' + termId + '-' + d); if (!el) return;
    var note = el.value.trim() || null;
    var r = await sb.from('term_weeks').update({ note: note }, { count: 'exact' }).eq('term_id', termId).eq('report_date', d);
    if (r.error) return C41.toast('저장 실패: ' + C41.errMsg(r.error));
    if (r.count === 0) return C41.toast('권한이 없습니다');
    C41.toast(C41.weekLabel(d) + ' ' + (note || '(비움)')); await loadSettings(); render();
  }
  // 학기별 구역 누계 수동값. 엑셀에서 넘어온 지난 학기 숫자는 DB 기록과 다르므로 손으로 넣는다.
  async function saveTermTotal(termId, parishId) {
    var el = document.getElementById('tt-' + termId + '-' + parishId); if (!el) return;
    var raw = el.value.replace(/,/g, '').trim();
    if (raw === '') return C41.toast('값을 넣으세요 (0 도 됩니다)');
    var n = parseInt(raw, 10);
    if (isNaN(n) || n < 0) return C41.toast('0 이상의 정수만');
    var r = await sb.from('term_totals').upsert({ term_id: termId, parish_id: parishId, total: n, updated_at: new Date().toISOString() }, { onConflict: 'term_id,parish_id' });
    if (r.error) return C41.toast('저장 실패: ' + C41.errMsg(r.error));
    C41.toast(parishId + ' ' + termId + ' 누계 ' + C41.fmt(n)); await loadSettings(); render();
  }
  ...
  // 구역장 계정은 아이디·비밀번호에 규칙이 있다: admin<구역번호> / <구역번호>00.
  // 구역을 고르는 순간 채워 주고, 고쳐 쓸 수 있게 둔다. 역할이 구역장이 아니면 구역 칸을 숨긴다.
  function syncStaffForm() {
    var roleEl = document.getElementById('staff-role'), pEl = document.getElementById('staff-parish');
    var idEl = document.getElementById('staff-id'), pwEl = document.getElementById('staff-pw');
    if (!roleEl || !pEl) return;
    var leader = roleEl.value === 'leader';
    pEl.hidden = !leader;
    if (leader && /^\d{4}$/.test(pEl.value)) {
      idEl.value = 'admin' + pEl.value;
      var sug = pEl.value + '00', dp = defaultPw();
      // 규칙값이 구역원 공용 비밀번호와 같으면(4133 이 그렇다) 채워 봐야 발급이 막힌다.
      // 비워 두고 다른 값을 넣으라고 알린다 — 막히는 값을 미리 채워 헛걸음시키지 않는다.
      if (dp && sug === dp) { pwEl.value = ''; pwEl.placeholder = '공용 비밀번호와 겹침 — 다른 6자리'; pwEl.focus(); C41.toast(pEl.value + ' 은 규칙값이 구역원 공용 비밀번호와 같습니다. 다른 여섯 자리를 넣으세요', 4000); }
      else pwEl.value = sug;
    }
  }
  async function addStaff() {
    ...
    var parish = role === 'leader' && pEl ? pEl.value : '';
    if (!/^[a-z][a-z0-9]{2,19}$/.test(id)) return C41.toast('아이디는 영문 소문자로 시작하는 3~20자');
    if (role === 'leader' && !/^\d{4}$/.test(parish)) return C41.toast('구역을 고르세요');
    var minPw = role === 'leader' ? 6 : 8;
    if (pw.length < minPw) return C41.toast('비밀번호는 ' + minPw + '자 이상');
    // 구역장 비밀번호가 구역원 공용 비밀번호와 같으면, 그 구역 구역원 누구나 구역장으로 들어와 명단을 고친다.
    // 4133 은 공용이 413300 이라 <구역번호>00 규칙과 정확히 겹친다 — 여기서 잡는다.
    if (role === 'leader' && dp && pw === dp) {
      return noteModal('비밀번호가 구역원 공용과 같습니다',
        '구역원 공용 비밀번호와 같은 값입니다.\n'
        + '이대로 발급하면 그 구역 구역원 누구나 구역장 계정으로 로그인해 명단을 고칠 수 있습니다.\n\n'
        + '다른 비밀번호를 넣어 주세요.');
    }
    var label = role === 'admin' ? '관리자' : role === 'leader' ? '구역장' : '교역자';
    if (!await C41.ask(label + ' 계정 발급',
        '아이디 ' + id + ' 로 ' + label + ' 계정을 만듭니다.\n\n'
        + (role === 'admin'
            ? '이 계정은 모든 구역의 명단·출석·계정을 다룹니다.\n첫 로그인 때 2단계 인증 등록 화면이 뜨고, 등록 전에는 자료가 보이지 않습니다.'
            : role === 'leader'
            ? parish + ' 구역의 출석 입력·명단·대시보드를 다룹니다. 다른 구역은 보이지 않습니다.\n계정 발급·직책 변경은 할 수 없습니다(교역자 전용).'
            : '이 계정은 전 구역 현황을 보기만 합니다. 명단·계정·설정은 바꿀 수 없습니다.\n2단계 인증은 요구하지 않습니다.'),
        { ok: '발급', danger: role === 'admin' })) return;
    busy(true);
    var failed = '';
    try {
      var req = { action: 'create_staff_account', login_id: id, password: pw, role: role };
      if (role === 'leader') req.parish_id = parish;
      await callAdmin(req);
    }
    catch (e) { failed = e.message; }
    busy(false);
  ...
  function settingsHtml() {
    ...
    A.terms.forEach(function (t) {
      ...
      if (ws.length) {
        ...
        ws.forEach(function (w) {
          ...
          h += '<span style="display:inline-flex;align-items:center;gap:3px">'
             + '<button class="btn sm ' + (act ? 'ghost' : 'danger') + '" data-act="toggleTermWeek" data-t="' + esc(t.id) + '" data-d="' + esc(w.report_date)
             + '" title="' + (act ? '이 날짜를 주차 탭에서 빼려면 누르세요' : '다시 켜려면 누르세요') + '">'
             + esc(C41.weekLabel(w.report_date)) + (act ? '' : ' (꺼짐)') + '</button>'
             // 과 번호 — 학기표의 수요일 칸 밑에 보인다. 엔터로 저장.
             + '<input id="wn-' + esc(t.id) + '-' + esc(w.report_date) + '" value="' + esc(w.note || '') + '" placeholder="과" maxlength="12" style="width:58px;padding:4px 6px;min-height:0" data-enter="saveWeekNote" data-t="' + esc(t.id) + '" data-d="' + esc(w.report_date) + '" title="과 번호 (예: 15과) — 엔터로 저장">'
             + '</span>';
        });
        h += '</div>';
        h += '<div class="desc" style="margin-top:-2px">날짜 옆 칸에 <b>과 번호</b>(예: 15과)를 넣고 엔터 — 학기표 대시보드의 그 주 칸 밑에 보입니다.</div>';
      }
      ...
      // 학기 누계(수동값). 엑셀에서 넘어온 지난 학기 숫자는 DB 합계와 다르다(4133: 5,101 vs 2,931).
      // 학기표 대시보드는 이 값이 있으면 이것을, 없으면 기록 합계를 보여 준다.
      var tots = {}; (A.termTotals || []).forEach(function (x) { if (x.term_id === t.id) tots[x.parish_id] = x.total; });
      h += '<details><summary style="cursor:pointer;color:var(--muted);font-size:13px">학기 누계 (구역별 수동값 · ' + Object.keys(tots).length + '/' + A.parishes.length + ' 입력)</summary>'
         + '<div class="kv" style="margin-top:6px">';
      A.parishes.forEach(function (pp) {
        h += '<label>' + esc(pp.id) + '</label><span class="form"><input id="tt-' + esc(t.id) + '-' + esc(pp.id) + '" inputmode="numeric" value="' + (tots[pp.id] != null ? esc(String(tots[pp.id])) : '') + '" placeholder="기록 합계 사용" style="max-width:120px" data-enter="saveTermTotal" data-t="' + esc(t.id) + '" data-p="' + esc(pp.id) + '">'
           + '<button class="btn sm ghost" data-act="saveTermTotal" data-t="' + esc(t.id) + '" data-p="' + esc(pp.id) + '">저장</button></span>';
      });
      h += '</div></details>';
      h += '</div>';
    });
    ...
    if (A.staff && A.staff.length) {
      A.staff.forEach(function (u) {
        h += '<div class="li"><span class="nm">' + esc(u.login_id) + '</span>'
          ...
          + (u.role === 'leader'
              ? '<span class="badge">' + esc(u.parish_id || '?') + '구역</span>'
              : u.role === 'pastor'
    ...
    h += '<div class="form" style="margin-top:10px">'
      ...
      + '<select id="staff-role" data-change="staffRole"><option value="pastor">교역자 (대시보드만)</option><option value="leader">구역장 (자기 구역)</option><option value="admin">관리자 (전권)</option></select>'
      + '<select id="staff-parish" data-change="staffParish" hidden><option value="">구역 선택</option>'
      + (A.parishes || []).map(function (p) { return '<option value="' + esc(p.id) + '">' + esc(p.id + (p.name && p.name !== p.id + '구역' ? ' ' + p.name : '')) + '</option>'; }).join('')
      + '</select>'
      ...
      + '<b>구역장</b>은 자기 구역만 다룹니다(입력·명단·대시보드). 구역을 고르면 <code>admin구역번호</code> / <code>구역번호00</code> 으로 채워집니다 — 고쳐 써도 됩니다. '
      + '구역원 공용 비밀번호와 같은 값은 발급을 막습니다.<br>'
  ...
  var ACTIONS = {
    ...
    saveWeekNote: function (el) { saveWeekNote(el.getAttribute('data-t'), el.getAttribute('data-d')); },
    saveTermTotal: function (el) { saveTermTotal(el.getAttribute('data-t'), el.getAttribute('data-p')); },
    addStaff: function () { addStaff(); },
    staffRole: function () { syncStaffForm(); },
    staffParish: function () { syncStaffForm(); },
